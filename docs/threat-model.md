...
| T19 | T | LLM hallucination in AI suggestions (invented skill/event, wrong type-framing, e.g. `maintain_and_share` on an unmastered skill) | `lib/ai/suggest.ts` `isValidSuggestion`: rejects any skill/event id not in `lib/domain/suggest.ts`'s context, any number outside the context, any type-framing violation, and titles are code-generated, never model text. Nothing survives → zero suggestions (`no_reliable_suggestion`), never a padded list. Template fallback only on call failure/offline. **Implemented (see `tests/suggest.test.ts`).** |
